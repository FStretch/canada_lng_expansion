...
Default scenario: `measurement_central`. Run at 2026-08-20 10:03 UTC. Numbers to one decimal. Inputs read-only. `calc_group` from `Asset Register:calc_group`.

## 1. Headline

- **Annual total:** 259.2 MtCO2e/yr
- **Lifecycle total:** 10193.7 MtCO2e (excludes legacy facilities)
- **Headline capacity (export chain only):** 95.1 mtpa (liquefaction; not summed with import regasification or other chains)
...
- **Scope 1+2:** 46.7 MtCO2e/yr (18.0%)  |  **Scope 3:** 212.5 MtCO2e/yr (82.0%)
- **Territorial:** CAN 48.0 (18.5%)  |  BUNK 14.8 (5.7%)  |  FOR 196.5 (75.8%)
...
| proposed | 75.7 | 200.1 | 7856.4 | 36.1 / 12.3 / 151.7 |
...
| early_proposed | export | 49.7 | 126.6 | 22.8 / 4.3 / 99.5 |

## 3. By chain

| chain | stages | mtpa | Annual Mt/yr | Lifecycle Mt | CAN / BUNK / FOR |
|---|---|---|---|---|---|
| export | upstream_production@CAN → pipeline_transport@CAN → liquefaction@CAN → shipping@BUNK → regasification@FOR → combustion@FOR | 95.1 | 250.0 | 9935.5 | 45.1 / 8.5 / 196.5 |
...
| marinvest_baie_comeau | 10.0 | export | proposed | early_proposed | gas_turbine | 40 (Parameters:lifecycle_years_default) | 7.20 | 25.5 | 1021.9 | 4.6 / 0.9 / 20.1 |  |
...
| liquefaction | CAN | 21.2 | 8.2% |
| shipping | BUNK | 8.5 | 3.3% |
| regasification | CAN,FOR | 2.8 | 1.1% |
| combustion | BUNK,CAN,FOR | 201.2 | 77.6% |

## 6. Scenario range

| scenario | upstream | Annual Mt/yr | Lifecycle Mt | CAN Mt/yr |
|---|---|---|---|---|
| inventory_as_reported | 0.22 | 257.0 | 10107.6 | 45.8 |
| measurement_central | 0.25 | 259.2 | 10193.7 | 48.0 |
| measurement_high | 0.26 | 260.0 | 10222.4 | 48.7 |
| near_term_methane_gwp20 | 0.33 | 265.1 | 10423.2 | 53.8 |
| howarth_high | 0.55 | 281.1 | 11054.3 | 69.9 |

## 7. Context

Canada territorial under `measurement_central` is **48.0 MtCO2e/yr**.

- **6.9%** of national inventory (694 Mt, inventory basis — excludes LULUCF).
- **11.5% to 10.5%** of the 2030 target range (417–455 Mt). Targets include land-use accounting; the 694 inventory figure does not.
- **24.0%** of the 2030 overshoot gap (200 Mt, projected emissions minus the target path).

## 8. Assumptions that move the number most

- **Upstream factor** (central 0.25): inventory_as_reported (0.22) → 257.0 Mt/yr (-2.2); howarth_high (0.55) → 281.1 Mt/yr (+21.9).
...
- **FID delay** (`fid_delay_mid`=5 yr inside lifespan): applies to 9 projects (75.7 mtpa export), currently 200.1 Mt/yr. Removing the delay would raise their annual emissions by roughly the ratio of lost ramp years (order-of-magnitude: several Mt/yr on the proposed export slate).
