...
Default scenario: `measurement_central`. Run at 2026-09-03 09:02 UTC. Numbers to one decimal. Inputs read-only. `calc_group` from `Asset Register:calc_group`.
...
10,000 draws, seed `20260828`. Physics 0.18s; pricing 0.04s. Kernel vs published panel max abs 7.1e-15 Mt. Liquefaction held at Emission Factors central. Howarth 0.55 is a named point, not a draw. Draws are on the headline scope (export chain). Each draw carries its own upstream and shipping factor, so its CH4 mass moves with it; damages are CO2 at SC-CO2 plus CH4 mass at SC-CH4, the same per-gas treatment as the central case.
